...
const GraphState = Annotation.Root({
  ...
  guardrail: Annotation<GuardrailFinding | undefined>({
    default: () => undefined,
    reducer: (_left, right) => right ?? undefined,
  }),
...
graphBuilder.addNode("answerGrounder", async (state: GraphStateType) => {
  const answer = answerGrounderTool({
...
graphBuilder.addNode("guardrail", async (state: GraphStateType) => {
  if (!state.answer) {
    return {
      guardrail: { ok: false, warnings: ["Missing answer"] },
    };
  }
  const guardrail = guardrailTool({ answer: state.answer.answer });
  return {
    guardrail,
...
graphBuilder.addEdge("answerGrounder", "guardrail");
graphBuilder.addEdge("guardrail", END);
...
export const runOrchestrator = async (
  ...
  return {
    ...
    guardrail:
      result.guardrail ?? ({ ok: false, warnings: ["Guardrail missing"] } as GuardrailFinding),
    logs: result.logs ?? [],
  };
};
